...
export const reencryptRequestMocked = async (
  ...
  const acl = new EthersT.Contract(
    ACL_ADDRESS,
...
export const getVersion = async (): Promise<string> => {
  const { status, result, message } = await fetch("http://localhost:3000/version", {
    method: "POST",
...
const encryptValues = async (
  ...
  return {
    handles: data.handles.map((h: EthersT.BytesLike) => EthersT.getBytes(h)),
    inputProof: EthersT.getBytes(data.inputProof),
  };
};

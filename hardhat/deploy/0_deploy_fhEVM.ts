...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const tfheExecutor = await hre.ethers.getContractAt(execArtifact.abi, TFHEEXECUTOR_ADDRESS);
  ...
  const zero = await impersonateAddress(hre, hre.ethers.ZeroAddress, hre.ethers.parseEther("100"));
  ...
  console.info(`==============================================`);
  console.info(`✅ ACL contract           : ${ACL_ADDRESS} version: ${await acl.getVersion()}`);
  console.info(`✅ TFHEExectuor contract  : ${TFHEEXECUTOR_ADDRESS} version: ${await tfheExecutor.getVersion()}`);
  console.info(`✅ KMSVerifier contract   : ${KMSVERIFIER_ADDRESS} version: ${await kms.getVersion()}`);
  console.info(`✅ InputVerifier contract : ${INPUTVERIFIER_ADDRESS} version: ${await input.getVersion()}`);
  console.info(`✅ Gateway contract       : ${GATEWAYCONTRACT_ADDRESS} version: ${await gateway.getVersion()}`);
  console.info(`==============================================`);
};
export default func;
